...
class DDIM(DDPM):
    ...
    def sample(self, n_sample: int, size, device) -> torch.Tensor:
        ...
        for i in range(self.n_T, 1, -1):
            ...
            # c1 is sigma_t in the paper
            c1 = self.eta * ((1 - self.alphabar_t[i] / self.alphabar_t[i - 1]) * (1 - self.alphabar_t[i - 1]) / (
                    1 - self.alphabar_t[i])).sqrt()
            # c2 is the coefficient of eps (2nd term) in the paper
            c2 = ((1 - self.alphabar_t[i - 1]) - c1 ** 2).sqrt()
            # AbeL: corresponds to eq. 12 in the DDIM paper https://arxiv.org/pdf/2010.02502.pdf
